...
for CUR_DIR in /host/${LOGDIR}/${NAME} /host/${DATADIR}/${NAME} /host/${CONFDIR}/${NAME} ; do
	...
	if [ "$CUR_DIR" == "/host/${CONFDIR}/${NAME}" ] ; then
	    mv -Rv /insteonmqtt/config.yaml.example /host/${CONFDIR}/${NAME}/config.yaml
	fi
        chmod 775 $CUR_DIR
	chgrp -R 0 $CUR_DIR
	chmod g+r -R $CUR_DIR
...
echo '10-local.rules:KERNEL=="ttyUSB?", ATTRS{idVendor}=="0403", ATTRS{idProduct}=="6001", NAME="insteon0", SYMLINK="insteon", GROUP="18", MODE="660"' > /host/etc/udev/rules.d/10-insteon.rules

chroot /host /usr/bin/podman create --name ${NAME} --net=host --device /dev/insteon --entrypoint /sbin/entrypoint.sh -v ${DATADIR}/${NAME}:/var/lib/insteonmqtt:z -v ${CONFDIF}/${NAME}:/etc/insteonmqtt -v ${LOGDIR}/${NAME}:/var/log/insteonmqtt ${IMAGE} /bin/start.sh
